...
## Summary
We apply poisson mash and mash to a subset of B cells from a single cell cytokines dataset, where the conditions represent different types of cytokine treatments ($R=25$). We apply both methods to the $8,568$ genes with at least $100$ reads across all cells, so the count data matrix $X$ is $8,568 \times 25$, where $X_{jr}$ represents the total counts of gene $j$ in all cells from condition $r$. 
...
\text{and} \;\; & \eta_j \sim MVN(0, \: \phi_j^2 I_R). \\
\end{align}



### Running ED step
To estimate the data-driven prior covariances $U_k$, we fit the poisson mash model without scaling parameters $w_l$ to a subset of genes ($\tilde{J} = 1367$) that are differentially expressed across conditions, which are identified based on a conditional multinomial goodness-of-fit test. 
```{r warning=FALSE, message=FALSE}
...
We then fit the poisson mash model to the count data from all the $8,568$ genes.
```{r warning=FALSE, message=FALSE}
