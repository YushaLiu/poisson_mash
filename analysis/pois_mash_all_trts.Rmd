...
### Model
We assume $\log(X_{jr}) \sim Pois(s_r \lambda_{jr})$, and $\tilde{\beta}_{jr} = \log(\lambda_{jr}) - \mu_j$, where $\mu_j$ is the gene-specific mean on the log scale shared by all conditions.
\begin{align}
