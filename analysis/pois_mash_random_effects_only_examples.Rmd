...
geneset1 <- idx.pois.mash[(!(names(idx.pois.mash) %in% names(idx.mash))) & (names(idx.pois.mash) %in% rownames(lfsr.mash))]

### randomly select 10 genes to plot
set.seed(500)
genes.to.plot <- names(geneset1)[sample(1:length(geneset1), 10, replace = FALSE)]
...
Now we present some examples of genes that get tesed in poisson mash only and are identified to be differentially expressed across treatments. 
```{r warning=FALSE, message=FALSE}
### genes that are identified to be differentially expressed only by poisson mash, but also get tested in mash
geneset2 <- idx.pois.mash[!(names(idx.pois.mash) %in% rownames(lfsr.mash))]

### randomly select 10 genes to plot
set.seed(500)
genes.to.plot <- names(geneset2)[sample(1:length(geneset2), 10, replace = FALSE)]

### a few examples
for(j in 1:10){
  create.plot(data=data, s=s, res.pois.mash=post, res.mash=mash.fit$result, genename=genes.to.plot[j], cols.trt=cols.trt, plot.mash=FALSE)
}
