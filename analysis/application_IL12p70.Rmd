...
### Eigenvector estimate of the data-driven covariance initialized by rank-3 PCA
eig.tPCA <- eigen(fit.ed$Ulist[[1]])
pve.tPCA <- eig.tPCA$values/sum(eig.tPCA$values)

v <- eig.tPCA$vectors[,1]
barplot(v/v[which.max(abs(v))], names = cell_types, cex.names = 0.6, las = 2, col = cols,
...
v <- fit.ed$ulist[[1]]
barplot(v/v[which.max(abs(v))], names = cell_types, cex.names = 0.6, las = 2, col = cols,
        main = paste0(names(fit.ed$ulist)[1], ": weight ", pi.post[11]))
